...
[You can find our final project report here!](https://upenn-embedded.github.io/final-project-s25-cat/)
...
| ------ | -------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------ |
| SRS-01 | Sensor Polling Frequency (Updated): Instead of polling, we implemented timer-based interrupts for the ultrasonic sensor, using timer 3. The system behavior confirmed that wall detection was responsive without lag. | Confirmed, wall detection was responsive without lag during testing with timer-based interrupts.              |
| SRS-02 | Trash Detection (Updated): We replaced the IR sensor with a YOLO machine learning model running on the Raspberry Pi. In testing, the robot reliably detected yellow blocks placed within view of the camera and triggered collection behavior appropriately. | Verified, robot successfully detected yellow blocks and triggered collection behavior during real-world tests. [▶ Watch our camera detecting the block video](https://drive.google.com/file/d/1n4UYuvp0TMSXZDLKCUhrddrH8jbGPYtZ/view?usp=sharing) |
| SRS-03 | Obstacle Avoidance Threshold: Wall detection using the ultrasonic sensor remained as planned. We manually validated behavior by observing that the robot stopped moving when approaching walls within about 20–30 cm. | Confirmed, robot stopped within the expected range when approaching walls, verified through manual tests.      |
| SRS-04 | Image Processing (Updated): The Raspberry Pi captured and processed images at approximately 1 frame per second. We observed that the robot responded to trash detection reliably. | Verified, robot processed images and responded to trash detection with no significant delay. [▶ Watch our camera perceiving distance of the block video](https://drive.google.com/file/d/12GPv1BvtwwBkfn28_EqNGDv7AikHxlrk/view?usp=sharing) |
| SRS-05 | PWM Motor Control Timing: Motor PWM control was implemented with Timer0 at a target of ~500 Hz. We observed smooth and consistent motor operation without perceptible jitter. | Confirmed, motor control was smooth with no perceptible jitter during operation. [▶ Watch our motor control (forward, backwards, speed) video](https://drive.google.com/file/d/1o-LlPXBNlsq7b0NR9ot6adOTAn2zIAc2/view?usp=sharing) |
| SRS-06 | SPI Communication Protocol (Updated): Instead of I2C, we implemented SPI communication between the Raspberry Pi and ATMega328PB for transmitting navigation directions. We confirmed low-latency updates from detection to motor action. | Verified, SPI communication provided low-latency updates from detection to motor action during tests. **SPI Dummy Value Test** ![SPI Dummy Value test](image/README/1744933511560.png) |
| SRS-07 | User Interrupt & Safety Response: An emergency stop button connected to PC0 was successfully integrated using a pin-change interrupt. During testing, the motors stopped within 100 ms of button press, as confirmed by manual tests. | Confirmed, motors stopped within 100 ms of button press during manual safety tests. [▶ Watch our motor rotation video](https://drive.google.com/file/d/1erBPFCrLf8zJfBMNw_j0bJg6JB4yxkOM/view?usp=sharing) |

**Confusion Matrix Normalized - Prequantized Model**  
![Confusion Matrix Normalized - prequantized model](image/README/confusion%20matrix%20normalized.jpg)

**Composite of Images - Prequantized Model**  
![Composite of images - prequantized model](image/README/composite%20of%20images.jpg)
...
| ID     | Description                                                                                                          | Validation Outcome                                                                                           |
| ------ | -------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------ |
| HRS-01 | Drive Motor Performance: The robot was able to drive forward while carrying a load. Although ramp testing was not conducted, the motors provided sufficient torque for smooth motion and start-up. A counterweight at the back successfully balanced the front shovel. We validated speed across 1 meter and observed no stalling or overheating. | Verified, the robot was able to move smoothly, with no stalling or overheating, during speed tests across 1 meter. |
| HRS-02 | Trash Detection via Camera instead of IR Sensor (Updated): We did not use an IR sensor. Instead, we used a forward-facing Raspberry Pi camera paired with a YOLO machine learning model to detect trash objects. The camera reliably identified yellow blocks placed in the robot’s path and triggered collection behavior accordingly. Detection performance was stable across multiple trials and lighting conditions. | Verified, YOLO model on the Raspberry Pi successfully detected trash in various lighting conditions. [▶ Watch our camera detecting the block video](https://drive.google.com/file/d/1n4UYuvp0TMSXZDLKCUhrddrH8jbGPYtZ/view?usp=sharing) |
| HRS-03 | Ultrasonic Sensor Distance & Accuracy: We placed obstacles at 10 cm, 50 cm, 100 cm, 200 cm, and 300 cm and recorded 10 readings at each distance. Measurement error was within ±1 cm for short range and ±5 cm for long range. The sensor consistently detected objects up to 3 meters without false negatives. | Verified, ultrasonic sensor showed consistent distance accuracy with minimal error and reliably detected objects up to 3 meters. |
| HRS-04 | Camera Setup: The Arducam was mounted securely and delivered live image captures at ≥480p resolution and 1 FPS. The camera remained stable during movement, and frame delivery to the Pi was confirmed during trash detection testing. | Confirmed, the camera delivered stable images at 480p resolution and maintained stable operation during testing. |
| HRS-05 | Battery and Power System: A power bank was used to supply the required motor and 5 V logic rails. Under high load, the 5 V line remained within 4.75–5.25 V, and no brown-outs or instability were observed. The robot ran continuously for at least 2 hours, exceeding the 1-hour requirement. | Verified, the robot successfully ran for over 2 hours under continuous load with stable voltage. |
| HRS-06 | Logic Level Interface (SPI Bus Hardware): SPI communication between the 5 V ATMega328PB and 3.3 V Raspberry Pi was implemented using a voltage divider. Oscilloscope readings showed clean signal transitions, and extended communication tests confirmed stability and accuracy without significant packet loss, even under motor noise. | Confirmed, SPI communication remained stable and accurate with no significant packet loss during testing. **SPI Dummy Value Test** ![SPI Dummy Value test](image/README/1744933511560.png) |
                                                                                                                      |
